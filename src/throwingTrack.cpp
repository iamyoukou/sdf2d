// When a player throws something, its track is drawn.
// I mimic this track-drawing algorithm in 2D environment.
//
// My method is:
// 1. In each frame, save the position (e.g. p0, p1, ...) of a particle into a
// queue.
// Note: I use std::vector instead of std::queue, because it is not allowed to
// access the arbitrary element of std::queue.
//
// 2. In each frame, if there are more than two points in this queue, draw a
// polyline with them.
//
// Option:
// Restricting the number of the track points can create a shooting-star effect.
//
// Search with the keyword "track points" for the code.
#include <iostream>
#include <vector>
#include <glm/glm.hpp>
...
void createPolygons() {
  ...
  object1.color = Scalar(164, 70, 152);
  object1.computeAabb();
...
void drawParticles() {
  for (int i = 0; i < particles.size(); i++) {
    ...
    circle(canvas, Point(ix, iy), particles[i].radius, particles[i].color, -1);
  }
}
...
void simulation() {
  ...
  while (frame < 600) {
    ...
    for (int i = 0; i < particles.size(); i++) {
      ...
      vector<Point> &tracks = particles[i].tracks;
      ...
      // if (tracks.size() > 100) {
      //   tracks.erase(tracks.begin(), tracks.begin() + 1);
      // }

      // draw track points
      if (tracks.size() > 2) {
        polylines(canvas, tracks, false, iWHITE, 2);
      }

      // draw particles
      circle(canvas, Point(ix, iy), particles[i].radius, particles[i].color,
             -1);

      // save the current position as a track point
      tracks.push_back(Point(ix, iy));
